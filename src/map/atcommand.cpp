...
// change include atcommand to run autoattack Bachnt
...
ACMD_FUNC(mapmove)
{
	...
	if(pc_get_group_level(sd) == 0) x = y = 0; // check @warp no coords Bachnt
...
{
	nullpo_retr(-1, sd);
	// FreeRO Modified - Additional check LapineUI opened
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PRONTERA,    155, 181 }, //  0=PRONTERA        	FreeRO Modified
		{ MAP_MORROC,      156,  93 }, //  1=Morroc
		{ MAP_GEFFEN,      119,  59 }, //  2=Geffen
		{ MAP_PAYON,       172, 101 }, //  3=Payon						FreeRO Modified
		{ MAP_ALBERTA,     117, 56 }, //  4=Alberta						FreeRO Modified
#ifdef RENEWAL
		...
		{ MAP_NIFLHEIM,     196, 179 }, // 13=Niflheim				FreeRO Modified
		{ MAP_LOUYANG,     217,  98 }, // 14=Louyang					FreeRO Modified
#ifdef RENEWAL
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		// FreeRO Modified - Synthesis & LapineUI
...
ACMD_FUNC(autoloot)
{
	double drate; // save autoloot to DB Bachnt
	int rate;
	nullpo_retr(-1, sd);
	...
	if (sd->state.autoloot) {
		if( Sql_Query( mmysql_handle, "REPLACE INTO `bachnt_autoloot`(`name`,`autoloot`) VALUES ('%s',%f);", sd->status.name, drate) != SQL_SUCCESS ){
			Sql_ShowDebug( mmysql_handle );
		}
		...
		if( Sql_Query( mmysql_handle, "DELETE FROM `bachnt_autoloot` WHERE `name` = '%s';", sd->status.name ) != SQL_SUCCESS ){
			Sql_ShowDebug( mmysql_handle );
		}
...
ACMD_FUNC(autolootitem)
{
	...
	case 1:
		...
		if( Sql_Query( mmysql_handle, "INSERT IGNORE INTO `bachnt_autolootid`(`name`,`itemid`,`combine`) VALUES ('%s',%u,%u);", sd->status.name, sd->state.autolootid[i],sd->status.account_id * sd->status.char_id + sd->state.autolootid[i] ) != SQL_SUCCESS ){
			Sql_ShowDebug( mmysql_handle );
		}
	...
	case 2:
		...
		if( Sql_Query( mmysql_handle, "DELETE FROM `bachnt_autolootid` WHERE `name` = '%s' AND `itemid` = %u;", sd->status.name, sd->state.autolootid[i] ) != SQL_SUCCESS ){
			Sql_ShowDebug( mmysql_handle );
		}
